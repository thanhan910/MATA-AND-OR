...
def greedyNETree1(agents, tasks, constraints, tree_info : list[Node], root_node_index=-1, eps=0, gamma=1, coalition_structure=[]):
    """
...
def greedyNETree2(agents, tasks, constraints, tree_info : list[Node], root_node_index=-1, eps=0, gamma=1, coalition_structure=[]):
    """
    GreedyNE algorithm for solving the problem when the tasks are organized in strictly alternating AND-OR tree (i.e. each OR node has only AND children, and each AND node has only OR children)
    """    
    re_assignment_count = 0
